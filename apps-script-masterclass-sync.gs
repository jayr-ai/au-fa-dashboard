 ...
 *
 * Setup:
 * 1. Open the FA | Webinar Lead Tracker spreadsheet
 * 2. Create a new Apps Script project (Extensions > Apps Script)
 * 3. Copy this code into Code.gs
 * 4. Set up trigger: Projects > Triggers > New trigger
 *    - Function: syncMasterclassData
 *    - Event: Time-driven > Day timer > 12:00 AM
 */
...
const SHEET_ID = '1g4h0IHwz0_BZ90nslU7NNKwIsENJw9hzgbk3A52dcQo'; // FA | Webinar Lead Tracker
...
// GitHub for dashboard JSON export
...
function readMasterclassSheet() {
  ...
  // Map columns: A=Opt-In Date, B=Client Name, C=Email, D=Phone, E=Source, F=Webinar Date
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Parse webinar date (Google Sheets may return string or Date object)
    let webinarDate;
    if (webinarDateRaw instanceof Date) {
      webinarDate = formatDate(webinarDateRaw);
    } else {
      // Try to parse as string (e.g., "Tue Aug 18")
    ...
    registrations.push({
      sync_id: generateSyncId(email, webinarDate),
      email: email,
...
function parseWebinarDate(dateStr) {
  // Parse dates like "Tue Aug 18" or "2026-08-18"
  try {
...
function generateSyncId(email, webinarDate) {
...
function syncToBigQuery(registrations) {
  const bq = BigQuery.newBigQueryResource();
...
function generateDashboardJSON(summary) {
  // Integrate with existing masterclass runs data
  // Merge sheet data with any other metrics (attended, deals, etc.)
  ...
  return {
    ...
    masterclassRegistrations: summary.map(run => ({
      ...
      attended: 0, // Placeholder - sync separately if tracked in sheet
      showUpRatePct: 0,
      vipUpgrade: 0,
      cashFromVip: 0,
      application: 0,
